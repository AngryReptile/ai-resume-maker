...
export default function SplitColumn({ previewData }: { previewData?: import("@/store/useResumeStore").ResumeData }) {
            ...
            <Reorder.Group axis="y" values={data.experience} onReorder={useResumeStore.getState().setExperienceOrder} className="space-y-8">
                {data.experience.map((exp) => (
                    <Reorder.Item key={exp.id} value={exp} className="relative group/exp paged-block">
                        ...
                        <EditableText value={exp.company} onSave={(val) => updateField('experience', 'company', val, exp.id)} placeholder="Company" className="text-sm font-semibold text-zinc-500 mb-3 block" />
                        <EditableText
            ...
            <Reorder.Group axis="y" values={data.education} onReorder={useResumeStore.getState().setEducationOrder} className="space-y-4">
                {data.education.map((edu) => (
                    <div key={edu.id} className="relative group/edu">
                        ...
                        <EditableText value={edu.degree} onSave={(val) => updateField('education', 'degree', val, edu.id)} placeholder="Degree" className="font-bold text-zinc-900 text-sm block" />
                        <EditableText value={edu.school} onSave={(val) => updateField('education', 'school', val, edu.id)} placeholder="School" className="text-zinc-600 text-xs mt-1 block" />
                        <div className="text-zinc-400 text-xs mt-1 font-medium italic flex items-center gap-1">
                ...
                {data.skills.map((skill) => (
                    ...
                    >
                        <EditableText
                            ...
                            placeholder="Skill"
                            className="text-sm text-zinc-600 leading-relaxed font-medium"
                        />
                        <button 
    ...
    const renderLanguages = () => (
        <Reorder.Item key="languages" value="languages" dragListener={false} dragControls={dragControls} className="group/section relative py-2 paged-block">
            <h3 className="text-xs font-black text-zinc-900 uppercase tracking-widest mb-2">{sectionLabels.languages}</h3>
            <EditableText
    ...
    const renderInterests = () => (
        <Reorder.Item key="interests" value="interests" dragListener={false} dragControls={dragControls} className="group/section relative py-2 paged-block">
            <h3 className="text-xs font-black text-zinc-900 uppercase tracking-widest mb-2">{sectionLabels.interests}</h3>
            <EditableText
    ...
    return (
        <div className="w-full flex min-h-full bg-white text-zinc-800 font-sans shadow-inner">
            {/* Left Column (1/3) */}
            <div className="w-1/3 p-8 border-r-2 border-zinc-100 flex flex-col gap-8 bg-zinc-50/50">
                <header>
                    ...
                    <div className="space-y-4 text-sm font-medium text-zinc-600">
                        <div>
                            <div className="text-xs text-zinc-400 uppercase tracking-wider mb-0.5 font-bold">Email</div>
                            <EditableText value={data.personalInfo.email} onSave={(val) => updateField('personalInfo', 'email', val)} placeholder="Email" />
                        </div>
                        <div>
                            <div className="text-xs text-zinc-400 uppercase tracking-wider mb-0.5 font-bold">Phone</div>
                            <EditableText value={data.personalInfo.phone} onSave={(val) => updateField('personalInfo', 'phone', val)} placeholder="Phone" />
                        </div>
                        <div>
                            <div className="text-xs text-zinc-400 uppercase tracking-wider mb-0.5 font-bold">Location</div>
